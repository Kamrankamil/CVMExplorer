...
import { FiChevronRight, FiHome, FiCheck, FiX, FiBox, FiCpu, FiUsers, FiClock } from 'react-icons/fi';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const ctx = document.getElementById('blockTransactionChart') as HTMLCanvasElement;
    if (ctx) {
      const chart = new Chart(ctx, {
        type: 'line',
        data: {
          labels: blocks.map(block => {
            const time = block.header.time ? ` (${displayDate(block.header.time.toISOString())})` : '';
            return `${time}`;
          }),
          datasets: [
            {
              ...
              backgroundColor: ['yellow', 'aqua', 'pink', 'lightgreen', 'lightblue', 'gold'], // Fill color
          ...
          <SimpleGrid minChildWidth="245px" spacing="12px">
            ...
            >
              <BoxInfo
                ...
                icon={FiBox}
                name="Latest Block"
                value={newBlock?.header.height || status?.syncInfo.latestBlockHeight}
                tooltext={newBlock?.header.time
  ...
  icon={FiClock}
  name="Block Time"
  value={newBlock?.header.time
    ? displayDate(newBlock?.header.time.toISOString())
    : status?.syncInfo.latestBlockTime
    ? displayDate(status?.syncInfo.latestBlockTime.toISOString())
    : ''}
    tooltext= {newBlock?.header.height || status?.syncInfo.latestBlockHeight}
/>
            </Link>
            <BoxInfo
              bgColor="orange.200"
        ...
        <Box display="flex" flexDirection={{ base: 'column', md: 'row' }} mt={8}>
        <Box bg={useColorModeValue('white', 'gray.800')} flex={{ base: '1', md: '3' }} mr={{ base: 0, md: 8 }} mb={{ base: 8, md: 0 }} style={{ width: '100%' }}>
  <canvas id="blockTransactionChart" style={{ width: '100%', height: '400px' }}></canvas>
</Box>
              ...
              <TabPanel>
                <TableContainer>
                  <Table variant='striped' colorScheme='teal'>
                    <Thead>
                      <Tr>
                        <th>    <CheckIcon color="green.500" /></th>
                        <Th>Height</Th>
                        <Th>App Hash</Th>
                        <Th>Txs</Th>
                        <Th>Time</Th>
  ...
  <Tooltip label="Success" aria-label="A tooltip">
    <CheckIcon color="green.500" />
        ...
        <Box display="flex" flexDirection={{ base: 'column', md: 'row' }} mt={8}>
  <Box flex={{ base: '1', md: '1' }} mr={{ base: 0, md: 8 }} mb={{ base: 8, md: 0 }}>
    <Box
      bg={useColorModeValue('light-container', 'dark-container')}
      shadow={'base'}
      borderRadius={4}
  ...
  return (
    ...
    >
      <Flex alignItems="center" justifyContent="space-between" width="100%">
        ...
        <Box display="flex" alignItems="center">
      <Tooltip label={tooltext} placement='left' bg='gray.300' color='black' shouldWrapChildren>
      ...
      <Flex justifyContent="center" alignItems="center"> {/* Centering horizontally */}
        <Stat>
